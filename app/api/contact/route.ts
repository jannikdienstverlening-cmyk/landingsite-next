...
export async function POST(request: NextRequest) {
  ...
  try {
    await getResend().emails.send({
      ...
      to: process.env.CONTACT_EMAIL ?? process.env.ADMIN_EMAIL ?? BUSINESS.email,
    ...
    return Response.json({ error: `Verzenden mislukt. Mail eventueel direct naar ${BUSINESS.email}.` }, { status: 500 })
  }
}
