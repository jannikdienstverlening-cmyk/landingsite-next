...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await getSupabase().from('orders').select('management_status, management_started_at, management_cancel_at_period_end, went_live_at')
  ...
  return Response.json({ management: data })
}
